...
use crate::Result;
use simple_fs::{list_files, read_to_string, SPath};
...
/// The content is formatted with line numbers and hashes (hashline format).
pub fn load_files_context(base_dir: impl Into<SPath>, globs: &[&str]) -> Result<Option<String>> {
    ...
    let res = if !files.is_empty() {
        ...
        for file in files {
            ...
            out.push_str(&format_hash_lines(&content, 1));
...
mod tests {
    ...
    fn test_load_files_context_simple() -> Result<()> {
        ...
        // -- Exec
        let context =
            load_files_context(&test_dir, &["src/**/*.rs"])?.ok_or("Should have context")?;

        // -- Check
        assert!(context.contains("<FILE_CONTENT path=\"src/lib.rs\">"));
        // Line 1 should have some hash like 1#HH:pub mod a;
